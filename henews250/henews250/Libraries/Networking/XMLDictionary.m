...
+ (NSString *)XMLStringForNode:(id)node withNodeName:(NSString *)nodeName
{
    ...
    {
        NSMutableArray *nodes = [NSMutableArray arrayWithCapacity:[(NSArray *)node count]];
    ...
    if (!top.attributes && !top.childNodes && !top.comments)
    {
        ...
        if (nodeName)
        {
            ...
            if (top.innerText && _collapseTextNodes)
            {
                ...
                {
                    parentNode[[(NSArray *)parentNode count] - 1] = top.innerText;
                }
                else
                {
...
- (NSString *)stringValueForKeyPath:(NSString *)keyPath
{
    ...
    {
        value = [(NSArray *)value count]? value[0]: nil;
    }
