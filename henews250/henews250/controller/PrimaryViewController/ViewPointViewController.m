...
- (void)readLocalOrderTiersDataOrRequest{
    NSMutableArray *orderArray = [[TierManager shareInstance] getOrderTierFromLocal:View];
    if (orderArray && orderArray.count != 0) {
        self.orderAry = orderArray;
    }else{
        NSString *newsPageUrl = [DEF_GetViewpage stringByAppendingString:[[CityManager shareInstance] getCity]];
...
- (void)requestDidFinishLoading:(NSString*)tag returnJson:(NSDictionary*)returnJson msg:(NSInteger)msg{
    if ([tag isEqualToString:@"newsPageData"]) {
        ...
        [[TierManager shareInstance]compareAndSave:newsAry key:View];
        self.orderAry = [[TierManager shareInstance] getOrderTierFromLocal:View];
    }
...
- (IBAction)classAddBtnSelect:(id)sender {
    if (tierManageView == nil) {
        tierManageView = [[TierManageView loadFromNib] initWithName:View];
